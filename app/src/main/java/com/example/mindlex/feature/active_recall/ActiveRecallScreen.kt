...
fun ActiveRecallScreen(
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Активное вспоминание") },
                navigationIcon = {
        ...
        Column(
            ...
            if (uiState.isLoading) {
                ...
                // Карточка слова — компактная, центрированная
                uiState.currentWord?.let { word ->
                    Card(
                        ...
                        Column(
                            ...
                            // Заголовок с переводом и кнопкой подсказки
                            Row(
                            ...
                            // Подсказка (показывается после нажатия на лампочку)
                            ...
                            // Показываем пример и транскрипцию ТОЛЬКО после проверки ответа
                ...
                // Поле ввода и кнопка проверки — скрываем если использована подсказка
                if (!uiState.hintShown && uiState.feedback == null) {
                    // Поле ввода — компактное, со скруглением
                    OutlinedTextField(
                ...
                // Обратная связь — показываем только если не использовалась подсказка
                uiState.feedback?.let { feedback ->
                    if (feedback.usedHint) {
                        // При подсказке — нейтральный UI без success/fail
                        HintUsedFeedbackCard(
                            correctAnswer = uiState.currentWord?.wordForeign,
                            onContinue = { viewModel.nextWord() }
                        )
                    } else {
                        // Без подсказки — стандартная обратная связь
                        FeedbackCard(
